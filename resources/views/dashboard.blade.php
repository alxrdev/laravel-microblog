                        ...
                        <div x-show="tab == 2">
                            ...
                            <ul class="space-y-4">
                                @foreach (Auth::user()->following()->get() as $following)
                                    <li>
                                        <a class="hover:text-stone-500" href="{{ route('posts.user', $following->id) }}">{{ $following->name }}</a>
                                        <a href="{{ route('toggleFollow', $following->id) }}"
                        ...
                        <div x-show="tab == 3">
                            ...
                            <ul>
                                @foreach (Auth::user()->likedPosts()->get() as $lpost)
                                    <li>
                                        <a class="hover:text-stone-500" href="{{ route('posts.show', $lpost->id) }}">
                                            {{ $lpost->title }}
                                        </a>
                                    </li>
                                @endforeach
