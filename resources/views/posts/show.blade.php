    ...
    <div class="my-14 flex flex-col">
        <div class="text-center">
            ...
            <p class="italic text-sm">by {{ $post->user->name }} <img class="ml-2 object-scale-down h-14 w-14 rounded-full inline"
                    src="http://localhost/images/test-image.png" alt="profile image"></p>
        ...
        <div class="flex mt-10">
            ...
            @if (Auth::user() && Auth::user()->id != $post->user->id)
                {{ __('You follow') }}:&nbsp;<a class="text-green-500 hover:text-green-700" href="{{ route('posts.user', $post->user->id) }}">
                    {{ $post->user->name }}</a>

                <a
                    href="{{ route('toggleFollow', $post->user->id) }}"
